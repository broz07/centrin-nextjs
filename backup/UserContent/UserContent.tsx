...
import { addUser } from '@centrin/utils/server/users';
import {
...
const UserContent: React.FC = () => {
	...
	const handlePasswordChange = (): void => {
		const password = (document.getElementById('password') as HTMLInputElement)
			.value;
		const passwordAgain = (
			document.getElementById('password-again') as HTMLInputElement
		).value;
	...
	const handleSubmit = async () => {
		...
		if (requiredError.length === 0 && !passwordError) {
			...
			if (added) {
				...
				notify(
					'Uživatele se nepodařilo přidat! 😥',
				...
				<DialogContent>
					<FormControl fullWidth>
						<TextField
							...
							helperText={
								requiredError.includes('name') ? 'Jméno nesmí být prázdné' : ''
							}
						/>
						<TextField
							margin="dense"
							...
							helperText={
								requiredError.includes('surname')
						...
						<TextField
							...
							helperText={
								requiredError.includes('username')
									? 'Přihlašovací jméno nesmí být prázdné'
									: ''
							}
						/>
						<TextField
