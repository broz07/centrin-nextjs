...
import { deleteUser, deleteUsers } from '@centrin/utils/users';
...
const RemoveUserDialog: React.FC<Props> = ({ open, close, userIds, resetSelected }) => {
    ...
    const handleRemove = async () => {
        ...
        if (!userIds) {
            updateToast(toast, 'Nepodařilo se odebrat uživatele.', NotificationType.ERROR, NotificationPosition.BR, 2000);
        ...
        if (deleted) {
            updateToast(toast, 'Záznamy byly úspěšně odebrány! 👍🏻', NotificationType.SUCCESS, NotificationPosition.BR, 2000);
        }
        else {
            updateToast(toast, 'Nepodařilo se odebrat uživatele. 😓', NotificationType.ERROR, NotificationPosition.BR, 2000);
        }
